...
export function UserManagement() {
  ...
  const [editUser, setEditUser] = useState({ username: '', phone: '', targetCal: '', targetWeight: '' });
  ...
  const startEditing = (user: any) => {
    ...
    setEditUser({
      ...
      phone: user.phone || '',
      targetCal: user.target_cal || user.targetCal || '',
  ...
  const handleEditUser = async (id: string) => {
    try {
      const res = await fetch(`/api/members/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...editUser, targetCal: Number(editUser.targetCal), targetWeight: Number(editUser.targetWeight) })
      });
      const data = await res.json();
      
      if (res.ok) {
        setUsers(users.map(u => u.id === id ? { ...u, ...editUser, target_cal: Number(editUser.targetCal), target_weight: Number(editUser.targetWeight) } : u));
          ...
          <CardContent>
            <form onSubmit={handleAddUser} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider ml-1">이름 (로그인 아이디)</label>
                <Input
                  required
                  placeholder="홍길동"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-slate-500 uppercase bg-slate-50/50 border-b border-slate-100">
              <tr>
                <th className="px-6 py-4 font-semibold tracking-wider">이름 (아이디)</th>
                  ...
                  {editingUserId === user.id ? (
                    <>
                      ...
                      <td className="px-6 py-4">
                        <Input
                          type="number"
                          value={editUser.targetCal}
                          onChange={e => setEditUser({ ...editUser, targetCal: e.target.value })}
                          className="w-24"
                        />
                      </td>
                      <td className="px-6 py-4">
                        <Input
                          type="number"
                          value={editUser.targetWeight}
                          onChange={e => setEditUser({ ...editUser, targetWeight: e.target.value })}
                          className="w-24"
                        />
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <Button size="sm" onClick={() => handleEditUser(user.id)} className="bg-indigo-600 hover:bg-indigo-700 text-white">
                            저장
                          </Button>
